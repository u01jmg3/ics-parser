...
class ICal
{
    ...
    public function __construct($files = false, array $options = array())
    {
        ...
        if ($files !== false) {
            ...
            foreach ($files as $file) {
                if ($this->isUrlOrFile($file)) {
                    $lines = file($file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
                    if ($lines === false) {
                        throw new \Exception("The URL '$file' does not exists.");
                    }
                } else {
                    throw new \Exception("The URL or file '$file' is not valid.");
                }
     ...
     * @return bool
     */
    private function isUrlOrFile($file)
    {
        return filter_var($file, FILTER_VALIDATE_URL) || file_exists($file);
    }
}
